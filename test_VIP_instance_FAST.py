...
#import models.videos.model_test_v2 as video3d
...
params = {}
params['filelist'] = 'vallist_ins.txt'
# params['batchSize'] = 24
params['imgSize'] = 240
params['cropSize'] = 240
params['cropSize2'] = 80
...
#params['filelist'] = 'temp'+str(args.batch_idx)+'.txt'
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        vip.VIPSet(params, is_train=True),
        batch_size=params['batchSize'], shuffle=True,
        num_workers=args.workers, pin_memory=True)

    val_loader = torch.utils.data.DataLoader(
        vip.VIPSet(params, is_train=False),
...
def test(val_loader, model, criterion, epoch, use_cuda):
    ...
    for batch_idx, (imgs_total, lbls, meta) in enumerate(val_loader):
        ...
        unique_lbls = np.unique(resize_lbl)
        lbl_set = palette[0 : len(unique_lbls), :]


        class_corres2 = []
        class_corres2.append(0)
        for i in range(len(unique_lbls)):
            for j in range(len(class_corres)):
                if unique_lbls[i] == class_corres[j][0]:
                    class_corres2.append(class_corres[j][1])
        ...
        count_lbls = np.zeros(len(unique_lbls))
            ...
            for k in range(resize_lbl.shape[1]):
                ...
                for t in range(len(unique_lbls)):
                    if unique_lbls[t] == pixellbl:
                        new_pixelbl = t
                        break
                count_lbls[new_pixelbl] += 1
                lbls_resize[0, j, k, new_pixelbl] = 1
        ...
        #for iter in range(total_frame_num - finput_num_ori):
            ...
            #corrfeat2   = torch.from_numpy(corrfeat2_now)
            ...
            corrfeat2 = corrfeat2.data.numpy()
            ...
            now_scores = np.zeros(len(lbl_set))
            now_scores2 = []
            class_corres3 = []
            indexes = np.zeros(len(lbl_set)).astype(np.int32)
            cnt_scores = np.zeros(len(lbl_set))
            ...
            for h in range(pred_mask.shape[0]):
                for w in range(pred_mask.shape[1]):
                    pixel_val = pred_mask[h, w]
                    now_scores[pixel_val] += predlbls_cp[h, w, pixel_val]
                    cnt_scores[pixel_val] += 1

            cnt_lbl = 0

            for t in range(len(cnt_scores)):
                if cnt_scores[t] > 0 :
                    now_scores[t] = now_scores[t] / cnt_scores[t]
                    now_scores2.append(now_scores[t])
                    class_corres3.append(class_corres2[t])
                    indexes[t] = cnt_lbl
                    cnt_lbl = cnt_lbl + 1

            for h in range(pred_mask.shape[0]):
                for w in range(pred_mask.shape[1]):
                    pred_mask[h, w] = indexes[pred_mask[h, w]]

            pred_mask = cv2.resize(pred_mask, (real_width, real_height), interpolation=cv2.INTER_NEAREST)

            predlbls_val = np.array(lbl_set)[np.argmax(predlbls_cp, axis=-1)]


            # predlbls = predlbls * 255
            # predlbls = cv2.resize(predlbls, (img_now.shape[0], img_now.shape[1]))
            ...
            for t in range(len(now_scores2) - 1):
                outstr = str(class_corres3[t + 1]) + ' ' + str(now_scores2[t + 1]) + '\n'
                f.write(outstr)
            f.close()

            scipy.misc.imsave(imname, np.uint8(img_with_heatmap))
            scipy.misc.imsave(imname2, np.uint8(predlbls_val))
            scipy.misc.imsave(eval_result_path, np.uint8(pred_mask))
